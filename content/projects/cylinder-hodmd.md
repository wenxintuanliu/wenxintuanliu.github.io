...
## 研究问题
测试快照为窗口为 source snapshots `1:301`,对应物理时间120-180。
圆柱尾迹包含稳定的基频和高阶谐波。标准 DMD 用相邻快照估计一步线性推进；HODMD 则把多帧历史放入延迟嵌入，用更高阶的时间信息提取 Koopman 近似模态 [cite:leclainche-hodmd]。

本项目展示延迟阶数`d=50` 的结果，回答两个问题：第一，HODMD 是否能给出清晰的尾迹频率和空间模态；第二，识别出的模态是否能在训练窗口之后稳定外推。
:::important Important 标题
实际测试了d=30、40、50、60、70(均处于0.1N-0.3N之间),发现提取出的频率十分相近并且重构误差相近，以最小误差d=50展示
:::
...
Fig 1 展示识别出的特征值与单位圆。特征值成对靠近单位圆，说明该圆柱尾迹主要由周期模态控制。
...
最终 physical RRMSE 为 `3.51e-3`；snapshot `301` 的局部全局相对 L2 误差为 `3.55e-3`，其中 `u` 分量为 `1.56e-3`，`v` 分量为 `1.48e-2`。
...
HODMD 外推使用同一组模态、增长率和频率继续推进快照302-400：
